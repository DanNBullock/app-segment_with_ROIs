...
tractNames=config['tractNames'].split(',')


availableROIs=glob(os.path.join(config['availableROIs'],'*.nii.gz'))

inputRequest=config['segRequests']
#example requests on single lines:

#'roiName any true'
#'roiName either_endpoint true'

#ok, but what about the case in which we want to do a multiseg...
tractRequests=[]
#if semicolons are being used to split multiple requests on a given line
if ';' in inputRequest:
    tractRequests=inputRequest.splitlines()
    #ok but now we need to split them into lists of lists
    tractRequests=[iTractRequests.split(';') for iTractRequests in tractRequests]
else: 
    tractRequests=[inputRequest.splitlines()]

for tractIterator,iTractSegs in enumerate(tractRequests):
    ...
    #turn the boolvec into a wmc, or add to it if one already exists
    if not 'outWmc' in locals():
        outWmc=wmaPyTools.streamlineTools.updateClassification(boolOut,tractName,existingClassification=None)
    else:
        outWmc=wmaPyTools.streamlineTools.updateClassification(boolOut,tractName,existingClassification=outWmc)
...
savemat(os.path.join(outDir,'classification.mat'),{ "classification": {"names": outWmc['names'], "index": outWmc['index'] }})
...
#It requires json construction, do it later.
#tcksDir=os.path.join(outDir,'wmc'
#wmaPyTools.streamlineTools.wmc2tracts(inputTractogram,classification,outdir)